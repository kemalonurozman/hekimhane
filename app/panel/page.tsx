...
export default function PanelPage() {
  ...
  // Kaydırma: sol kenardan (≤28px) sağa kaydır → menü açılır; menü açıkken sola kaydır → kapanır.
  // Düğmeye basmadan sekme değiştirmeyi kolaylaştırır. Dikey kaydırmayı bozmaz (yalnız yatay ve ≥60px).
  useEffect(() => {
    if (!isMobile) return;
    let sx = 0, sy = 0, edge = false, tracking = false;
    const start = (e: TouchEvent) => { const t = e.touches[0]; if (!t) return; sx = t.clientX; sy = t.clientY; edge = sx <= 28; tracking = true; };
    const end = (e: TouchEvent) => {
      if (!tracking) return; tracking = false;
      const t = e.changedTouches[0]; if (!t) return;
      const dx = t.clientX - sx, dy = t.clientY - sy;
      if (Math.abs(dx) < 60 || Math.abs(dy) > Math.abs(dx) * 0.8) return;
      if (dx > 0 && edge && !mobileMenuOpen) setMobileMenuOpen(true);
    ...
    <div style={{ display: 'flex', minHeight: '100vh', fontFamily: '-apple-system, BlinkMacSystemFont, "SF Pro Display", "Segoe UI", sans-serif' }}>
      ...
      {isMobile && (
        <div style={{
          ...
          background: T.navy, display: 'flex', alignItems: 'center', gap: 10, padding: '0 12px 0 8px'
        }}>
          {/* Menü SOLDA — standart mobil düzen, başparmağa yakın, sağ üstteki site
              menüsüyle karışmaz. Sol kenardan sağa kaydırmak da açar (yukarıdaki effect). */}
          <button onClick={() => setMobileMenuOpen(o => !o)}
            aria-label={mobileMenuOpen ? 'Menüyü kapat' : 'Menüyü aç'} aria-expanded={mobileMenuOpen}
            style={{
              flexShrink: 0, display: 'inline-flex', alignItems: 'center', gap: 7, minHeight: 36,
              background: mobileMenuOpen ? 'rgba(255,255,255,.24)' : 'rgba(255,255,255,.13)', border: '1px solid rgba(255,255,255,.22)',
              borderRadius: 9, cursor: 'pointer', color: 'white', padding: '7px 11px',
          ...
          <span style={{ color: 'white', fontWeight: 800, fontSize: 15, minWidth: 0, flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
      ...
      {isMobile && (
        <nav style={{
          ...
          {/* Menü SOLDA — üst bardaki düğmeyle aynı tarafta; kalan sekmeler
              (profil düzenleme, takvim, HekimKart, makaleler…) çekmecede */}
          <button onClick={() => setMobileMenuOpen(o => !o)} aria-label="Menü" aria-expanded={mobileMenuOpen}
